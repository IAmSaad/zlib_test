...
## Info
Compresses an input file using zlib's two distinct compression levels namely `Z_DEFAULT_COMPRESSION` (provideds optimal speed and compression tradeoff) and `Z_BEST_COMPRESSION` (gives best compression results), reports the output compressed file sizes and compression ratios along with the time taken to do both compressions.
